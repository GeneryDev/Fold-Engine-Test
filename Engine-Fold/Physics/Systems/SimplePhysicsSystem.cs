...
public class SimplePhysicsSystem : GameSystem
{
    ...
    private void CollisionDetectionAndResponse()
    {
        ...
        while (_physicsObjects.Next())
        {
            ...
            //Apply gravity
            if (!physics.Static)
                physics.ApplyForce(Gravity * physics.GravityMultiplier * physics.Mass, default,
                    ForceMode.Continuous);
            if (DrawCollisionGizmos)
                Scene.DrawGizmo(transformPosition, transformPosition + physics.Velocity.Normalized() * 1,
                    Color.Gold, zOrder: 1);

            //Skip if no collider
            Collider collider = default;
            if (_physicsObjects.HasCoComponent<Collider>()) collider = _physicsObjects.GetCoComponent<Collider>();
            ...
            while (_nearbyIntersections.Count > 0)
            {
                ...
                for (; i < _nearbyIntersections.Count; i++)
                    if (_nearbyIntersections[i].Intersections != null)
                        break;
    ...
    private void CalculateIntersections(Vector2[] colliderVertices, Vector2 velocity)
    {
        for (int i = 0; i < _nearbyIntersections.Count; i++)
        {
            ...
            var other = new Entity(Scene, data.Other);
            ...
            if (data.Intersections != null)
            {
                ...
                foreach (Polygon.PolygonIntersectionVertex[] intersection in data.Intersections)
                {
                    ...
                    for (int j = 0; j < intersection.Length; j++)
                    {
                        ...
                        if (current.IsFromB && next.IsFromB) data.ContactPoint = current.Position;
                    }
                }
    ...
    private void CollisionResponse(IntersectionData data, Entity entity)
    {
        ...
        foreach (Polygon.PolygonIntersectionVertex[] intersection in data.Intersections)
        {
            for (int i = 0; i < intersection.Length; i++)
            {
                ...
                //Draw gizmos
                if (DrawCollisionGizmos) Scene.DrawGizmo(current.Position, next.Position, Color.Fuchsia, zOrder: 1);
            }
        ...
        if (!largestMoveDepth.Equals(float.NaN) && maxSurfaceNormalFaceLength > 0)
        {
            ...
            if (!physics.Static && !(collider.IsTrigger || otherCollider.IsTrigger))
            {
                ...
                if (AttemptDisplacement(entity,
                        -collision.Direction.Normalized() *
                        collision.DirectionDepth /*collision.Normal.Normalized() * collision.NormalDepth*/,
            ...
            if (!physics.Static)
            {
                if (UseVerletIntegration)
                    transform.Position = oldPos
                                         + physics.Velocity * Time.FixedDeltaTime
                                         + 0.5f
    ...
    private void ApplyContactForces(
        ref Physics physicsA,
        ref Physics physicsB,
        Vector2 normal,
        float friction,
        float restitution)
    {
        ...
        if (UseVerletIntegration) physicsA.Velocity += (normalForce + frictionForce) / physicsA.Mass;
    ...
    private void ApplyAndResetForces()
    {
        ...
        while (_physicsObjects.Next())
        {
            ...
            {
                if (UseVerletIntegration)
                    physics.Velocity += (physics.AccelerationFromForce + physics.PreviousAcceleration)
                                        * 0.5f
                                        * Time.FixedDeltaTime;
