...
namespace FoldEngine.Physics {
    [GameSystem("fold:gizmos.collider", ProcessingCycles.Render, true)]
    public class ColliderGizmoRenderer : GameSystem {
        ...
        public override void OnRender(IRenderingUnit renderer) {
            ...
            while(_colliders.Next()) {
                var transform = _colliders.GetCoComponent<Transform>();
        ...
        public static void DrawColliderGizmos(Entity entity) {
            if(entity.HasComponent<Collider>())
                DrawColliderGizmos(entity.Scene, entity.Transform, ref entity.GetComponent<Collider>());
        }

        public static void DrawColliderGizmos(Scene scene, Transform transform, ref Collider collider) {
            var colliderColor = new Color(102, 226, 148);
            switch(collider.Type) {
                ...
                case ColliderType.Mesh: {
                    ...
                    bool first = true;
                    foreach(Vector2 localVertex in scene.Resources.Get<Mesh>(ref collider.MeshIdentifier, Mesh.Empty)
                        .GetVertices()) {
