...
public class ResourceIndex
{
    ...
    public void Update()
    {
        ...
        foreach (Type type in registry.GetAllTypes())
        {
            ...
            ScanGroups(groups, paths, groupsNeedExpanding,
                Path.Combine("resources", resourceAttribute.DirectoryName, GroupDirectoryName), resourceAttribute);
            ...
            while (groupsNeedExpanding.Count > 0)
            {
                ...
                foreach (string id in groupsNeedExpanding)
                {
                    groupId = id;
                    break;
                }
        ...
        foreach (Type type in registry.GetAllTypes())
        {
            ...
            foreach (KeyValuePair<string, string> id in paths) Console.WriteLine($"    - {id.Key} at {id.Value}");
        }
        ...
        foreach (Type type in registry.GetAllTypes())
        {
            ...
            foreach (KeyValuePair<string, HashSet<string>> group in groups)
            {
                ...
                foreach (string value in group.Value) Console.WriteLine($"      - {value}");
            }
        }
#endif
    }
        ...
        string relativeTo = null)
    {
        ...
        {
            foreach (string entry in Data.In.ListEntries(path))
                ScanGroups(groups, resources, groupsNeedExpanding, Path.Combine(path, Path.GetFileName(entry)),
                    resourceAttribute, relativeTo);
        }
        else if (Data.In.Exists(path))
        {
            string extension = Path.GetExtension(path);
            ...
            {
                string id = "#"
                            + Path.ChangeExtension(path.Substring(relativeTo.Length + 1), null)
                                .Replace(Path.DirectorySeparatorChar, '/');

                try
                {
                    HashSet<string> values = ParseGroup(id, path, resources, resourceAttribute,
                        out bool needsExpanding);
                    groups[id] = values;
                    if (needsExpanding) groupsNeedExpanding.Add(id);
                }
                catch (Exception x)
                {
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine("Error trying to load "
                                      + resourceAttribute.DirectoryName
                                      + " group: "
                                      + id
                                      + "\n"
                                      + x.Message);
    ...
    private static HashSet<string> ParseGroup(
        string id,
        string path,
        Dictionary<string, string> resources,
        ResourceAttribute resourceAttribute,
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            Console.WriteLine("Unknown value in "
                                              + resourceAttribute.DirectoryName
                                              + " group "
                                              + id
                                              + ": "
    ...
    public bool Exists(Type type, string identifier)
    {
        return _identifierToPathMap.ContainsKey(type) && _identifierToPathMap[type].ContainsKey(identifier)
               || _groups.ContainsKey(type) && _groups[type].ContainsKey(identifier);
    }
    ...
    {
        return _identifierToPathMap.ContainsKey(type) && _identifierToPathMap[type].ContainsKey(identifier)
            ? _identifierToPathMap[type][identifier]
            : null;
    }
