...
using Convert = Shard.Scripts.Instructions.Convert;

namespace FoldEngine.Graphics;

public class FontManager
{
    private static readonly BasicExpressionLanguage.Compiler FontContextCompiler;
    private readonly IGameCore _core;
    private readonly Dictionary<string, IFont> _fonts = new Dictionary<string, IFont>();
    ...
    public BitmapFont LoadFont(FontDefinition definition)
    {
        ...
        if (setName != null)
        {
            if (!_fonts.ContainsKey(setName)) _fonts[setName] = new FontSet();

            if (_fonts[setName] is FontSet fontSet)
                fontSet.AddFont(bitmapFont, bitmapFont.DefaultSize);
            else
                throw new InvalidOperationException(
                    $"Cannot create a font set named {setName}: another non-set font with that name already exists");
        }
        ...
        if (root["characters"] is JArray charsArr)
            foreach (JObject rawCharEntry in charsArr)
            {
                ...
                _core.Resources.Load<Texture>(ref sourceIdentifier, t =>
                {
                    ...
                    Func<char, int> heightEvaluator = EvaluatorInt(rawCharEntry["height"]);
                    Func<char, int> ascentEvaluator = EvaluatorInt(rawCharEntry["ascent"]);
                    Func<char, int> widthEvaluator = EvaluatorInt(rawCharEntry["width"]);
                    Func<char, int> advancementEvaluator = EvaluatorInt(rawCharEntry["advancement"]);

                    var rawUVArr = rawCharEntry["uv"] as JArray;

                    Func<char, int> uvXEvaluator = EvaluatorInt(rawUVArr[0]);
                    Func<char, int> uvYEvaluator = EvaluatorInt(rawUVArr[1]);

                    for (char c = start; c <= end; c++)
                    {
                        var glyphInfo = new GlyphInfo { NotNull = true };
    ...
    private static Func<char, int> EvaluatorInt(JToken token)
    {
        switch (token.Type)
        {
            ...
            case JTokenType.String:
            {
                ...
                FontContextCompiler.OnEnd = sw => sw.Write<Convert.I32>();
    ...
    private static Func<char, float> EvaluatorFloat(JToken token)
    {
        switch (token.Type)
        {
            ...
            case JTokenType.String:
            {
                ...
                FontContextCompiler.OnEnd = sw => sw.Write<Convert.F32>();
                Script s = FontContextCompiler.Compile(raw);
    ...
    public void LoadAll()
    {
        foreach (string fontName in _core.ResourceIndex.GetIdentifiers<FontDefinition>())
        {
            ...
            _core.Resources.Load<FontDefinition>(ref identifier, d => { LoadFont((FontDefinition)d); });
        }
    }
