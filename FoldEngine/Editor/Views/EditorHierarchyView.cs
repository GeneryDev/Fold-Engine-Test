    ...
    public class EditorHierarchyView : EditorView {
        ...
        public override void Render(IRenderingUnit renderer) {
            ...
            if(ContentPanel.Button("New Entity", 14).IsPressed()) {
                ((EditorEnvironment) ContentPanel.Environment).TransactionManager.InsertTransaction(new CreateEntityTransaction(-1));
            }
        ...
        private void RenderEntity(ref Transform transform, GuiPanel panel, IRenderingUnit renderer, int depth = 0) {
            ...
            switch(button.GetEvent(out Point p)) {
                case HierarchyElement<long>.HierarchyEventType.Expand:
                    (panel.Environment as EditorEnvironment)?.GetView<EditorHierarchyView>().Hierarchy.ExpandCollapse(entityId);
                    break;
            ...
            contextMenu.Show(point, (m) => {
                ...
                if(m.Button("Delete", 9).TextMargin(20).TextAlignment(-1).IsPressed()) {
                    ...
                    if(ContentPanel.Environment.Scene.Components.HasComponent<Transform>(id)) {
                        ContentPanel.Environment.Scene.Components.GetComponent<Transform>(id).DumpHierarchy(EntitiesToDelete);
                    }
                ...
                if(m.Button("Create Child", 9).TextMargin(20).TextAlignment(-1).IsPressed()) {
                    ((EditorEnvironment) ContentPanel.Environment).TransactionManager.InsertTransaction(new CreateEntityTransaction(id));
                }
            }, 120);
        }
    }
